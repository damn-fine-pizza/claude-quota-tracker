...
export function planQueue(args: { tasks: Task[]; meta: Map<number, TaskScheduleMeta>; mode: QueueMode; nowMs: number; cutoffMs: number | null; estimates: Map<number, number>; routing?: RoutingPolicy; reserveProfile?: string | null }): QueueDecision[] {
  ...
  return candidates.map((task) => {
    ...
    return { taskId: task.id, ok: blocked === "eligible" && !reserved, reasonCode: reserved?"reserved_profile":blocked, estimateTokens, providerId: m.providerId, profileId: profile };
  });
}
