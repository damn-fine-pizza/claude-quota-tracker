...
export async function main(argv: string[]): Promise<void> {
  ...
  switch (cmd) {
    ...
    case "queue": { if(argv[1]==="run"){ const ok=await runPacedOnce(); process.exitCode=ok?0:1; return; } const { planQueue } = await import("./queue-planner.js"); const { Store } = await import("./store.js"); const { SchedulerMetaStore } = await import("./scheduler-meta.js"); const store=new Store((await import("./config.js")).DB_PATH); const meta=new SchedulerMetaStore(); try { const tasks=store.listTasks(["queued","carried_over"]); const m=new Map(tasks.map(t=>[t.id,meta.getOrDefault(t.id)])); console.log(JSON.stringify(planQueue({tasks,meta:m,mode:argv[2]==="manual"?"manual":"priority",nowMs:Date.now(),cutoffMs:null,estimates:new Map(tasks.map(t=>[t.id,m.get(t.id)!.estimatedTokens??0]))}),null,2)); } finally {meta.close();store.close();} return; }
