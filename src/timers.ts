...
export class TimerStore {
  ...
  constructor(path = DB_PATH) { this.db = new DatabaseSync(path); this.db.exec("PRAGMA foreign_keys = ON"); this.db.exec("CREATE TABLE IF NOT EXISTS timers (id INTEGER PRIMARY KEY AUTOINCREMENT, kind TEXT NOT NULL CHECK(kind IN ('timestamp','cron')), expression TEXT NOT NULL, task_id INTEGER, paused INTEGER NOT NULL DEFAULT 0, created_ts INTEGER NOT NULL, updated_ts INTEGER NOT NULL)"); }
